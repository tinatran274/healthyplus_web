...
export const addP = async () => {
  const docRef = await addDoc(collection(db, "dish"), {
    ...
    recipe: [
      "1. Ức gà dùng muối hoặc chanh khử sạch mùi tanh sau đó rửa lại bằng nước sạch và để ráo nước.",
...
export const getAllProduct = async () => {
  ...
  const querySnapshot = await getDocs(
    collection(db, "product").withConverter(ProductConverter)
  );
...
export const addProductToCart = async (uid, pid, num) => {
  ...
  if (docSnap.exists()) await updateDoc(cartRef, dataToUpdate);
  else await setDoc(cartRef, dataToUpdate);
};
...
export const addReplyCmt = async (cid, uid, date, content) => {
  ...
  if (docSnap.exists()) await updateDoc(listRepRef, dataToUpdate);
  else await setDoc(listRepRef, dataToUpdate);
};
...
export const getReply = async (cid) => {
  ...
  const mergedObject1 = listRep.reduce(
    (acc, curr) => ({ ...acc, ...curr }),
    {}
  );
