...
import { getFirestore,  doc, setDoc, getDoc, collection, updateDoc, addDoc, getDocs} from "https://www.gstatic.com/firebasejs/9.22.0/firebase-firestore.js";
...
export const addComment = async (uid, did, date, content) => {
...
export const addRating = async (uid, did, rating) => {
    const dishRef = doc(db, "dish", did);
...
export const getComment = async (did) => {  
    const list = [];
    const listComment = [];
    const dishRef = doc(db, "dish", did);
...
export const getUserRating = async (uid, did) => {
    const dishRef = doc(db, "dish", did);
    const rateRef = doc (dishRef, "rating", uid)
...
export const getAllRating = async (did) => {
    const listRate = []
    const dishRef = doc(db, "dish", did);
...
export const getNumRating = async (did) => {
    const listRate = []
    const dishRef = doc(db, "dish", did);
    const querySnapshot = await getDocs(collection(dishRef, "rating"))
    querySnapshot.forEach((doc) => {
