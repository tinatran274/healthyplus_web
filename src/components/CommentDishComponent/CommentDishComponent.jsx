...
const CommentDishComponent = ({idDish}) => {
    ...
    const getDish = async () => {
        ...
        setListCmtDetail (await UserService.getComment(idDish));
    }
    const handleAuth = () => {
        onAuthStateChanged(auth, async (user) => {
            if (user) {
                const userData = await UserService.getDetailUser(user.uid);
                const userRating = await UserService.getUserRating(user.uid, idDish);
    ...
    const handleAddCmt = () => {
        UserService.addComment(userData.id, dish.id, getDateToday(), value);
        setValue("")
        message.success()
    }
    const handleAddRating = () => {
        UserService.addRating(userData.id, dish.id, rating);
        message.success()
    }
